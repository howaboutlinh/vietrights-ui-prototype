...
5. Add `GEMINI_API_KEY`, `GEMINI_CHAT_MODEL`, `GEMINI_EMBEDDING_MODEL`, `DATABASE_URL`, `RAG_MATCH_COUNT`, `RAG_MATCH_THRESHOLD`, and `EMBEDDING_DIMENSION` in Render Environment. `DATABASE_URL` must be the Supabase Session Pooler URL and uses the psycopg 3 driver (`postgresql+psycopg://`).
